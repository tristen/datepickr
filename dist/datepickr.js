;(function(global) {
  var Datepickr = (function() {
    ...
    function calendarHover() {
      ...
      this.calendarBody.addEventListener('mouseleave', function(e) {
        rangeSelect.call(this, e.target);
      }.bind(this));
    }
    ...
    function rangeSelect(el) {
      ...
      if (el.getAttribute('data-target') && indexes.length) {
        ...
        if (indexes.length === 1) {
          indexes = days.slice(indexes[0], indexes[0]);
        } else {
          // Snap to range if element in mouseover is not a day.
          indexes = days.slice(indexes[0], indexes[1]);
        }
      }

      // Assign a range class to anything in range.
